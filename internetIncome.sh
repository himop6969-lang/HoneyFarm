...
if [[ "$1" == "--start" ]]; then
  ...
  # ── MULTI-ACCOUNT HONEYGAIN: load honeygain.txt ─────────────────────────
  HONEYGAIN_ACCOUNTS=()
  honeygain_accounts_file="honeygain.txt"
  if [ "$USE_HONEYGAIN_MULTI" = true ]; then
    if [ ! -f "$honeygain_accounts_file" ]; then
      echo -e "${RED}honeygain.txt not found! Create it with email:password per line. Exiting..${NOCOLOUR}"
      exit 1
    fi
    sed -i 's/\r//g' "$honeygain_accounts_file"
    while IFS= read -r hg_line || [ -n "$hg_line" ]; do
      [[ "$hg_line" =~ ^\s*# ]] && continue
      [[ -z "$hg_line" ]] && continue
      HONEYGAIN_ACCOUNTS+=("$hg_line")
    done < "$honeygain_accounts_file"
    if [ ${#HONEYGAIN_ACCOUNTS[@]} -eq 0 ]; then
      echo -e "${RED}honeygain.txt is empty or has no valid entries. Exiting..${NOCOLOUR}"
      exit 1
    fi
    echo -e "${GREEN}Honeygain multi-account: ${#HONEYGAIN_ACCOUNTS[@]} account(s) loaded from $honeygain_accounts_file${NOCOLOUR}"
    if [ -z "$HONEYGAIN_DEVICES_PER_ACCOUNT" ]; then
      HONEYGAIN_DEVICES_PER_ACCOUNT=10
    fi
  fi
  # Global proxy counter for multi-account slot tracking
  _HG_PROXY_COUNTER=0
  ...
  if [ "$USE_PROXIES" = true ]; then
    ...
    # ── Multi-account Honeygain injection ───────────────────────────────
    if [ "$USE_HONEYGAIN_MULTI" = true ] && [ ${#HONEYGAIN_ACCOUNTS[@]} -gt 0 ]; then
      echo -e "${GREEN}Multi-account Honeygain: ${#HONEYGAIN_ACCOUNTS[@]} accounts x $HONEYGAIN_DEVICES_PER_ACCOUNT proxies each${NOCOLOUR}"
      _proxy_slot=0
      for hg_entry in "${HONEYGAIN_ACCOUNTS[@]}"; do
        HG_ACC_EMAIL="${hg_entry%%:*}"
        HG_ACC_PASS="${hg_entry#*:}"
        # Trim whitespace
        HG_ACC_EMAIL="${HG_ACC_EMAIL//[[:space:]]/}"
        HG_ACC_PASS="${HG_ACC_PASS//[[:space:]]/}"
        export HONEYGAIN_EMAIL="$HG_ACC_EMAIL"
        export HONEYGAIN_PASSWORD="$HG_ACC_PASS"
        echo -e "${GREEN}  -> Account: $HONEYGAIN_EMAIL${NOCOLOUR}"
        _dev_count=0
        while IFS= read -r line || [ -n "$line" ]; do
          if [[ "$line" =~ ^[^#].* ]]; then
            if [ "$_dev_count" -ge "$_proxy_slot" ] && [ "$_dev_count" -lt "$(( _proxy_slot + HONEYGAIN_DEVICES_PER_ACCOUNT ))" ]; then
              i=$(( i + 1 ))
        ...
        _proxy_slot=$(( _proxy_slot + HONEYGAIN_DEVICES_PER_ACCOUNT ))
      done
    else
      # Original single-account proxy loop
      while IFS= read -r line || [ -n "$line" ]; do
        if [[ "$line" =~ ^[^#].* ]]; then
          i=`expr $i + 1`
          start_containers "$i" "$line"
